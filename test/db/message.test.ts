...
test("Message queries", async (t) => {
  ...
  await t.test("list", async (t) => {
    await t.test("Should list messages correctly when no filter is provided", async () => {
      const user1 = await prisma.user.findFirst({ where: { email: "user1@example.com" } });
      const user2 = await prisma.user.findFirst({ where: { email: "user2@example.com" } });

      let result = await messageQueries.list(prisma, { limit: 1, senderId: user1.id, receiverId: user2.id });
      result = result.map((r) => ({ senderId: r.senderId, receiverId: r.receiverId, content: r.content }));

      assert.deepEqual(result, [
        { senderId: user2.id, receiverId: user1.id, content: `Message 2 from User ${user2.id} to User ${user1.id}` },
        {
          senderId: user2.id,
          receiverId: user1.id,
